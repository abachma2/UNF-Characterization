...
BU=[20,25,30,35,40,45,50,55,60] #Burnup values used, these are hardcoded in 
EN = np.genfromtxt("EN_values.txt",usecols=(0,1,2,3,4,5)) #Enrichment values calculated in origen_script.py and saved to the file specified

CoolingTimes = 6
initSkip = 6
numNB = 4 # number of neutron energy bins
numGB = 2049 #number of gamma energy bins
data = np.array([])
data_string = ''
    ...
    for x in range(0,len(BU)):
        ...
        for y in np.nditer(en_value):       
            ...
            for t in range(CoolingTimes):
                ...
                GSkip = GSkip + numGB + 2 #Skips to the next cooling time in the flie
                ...
                NSkip = NSkip + numNB + 1 #Skips to the next cooling time in the file
